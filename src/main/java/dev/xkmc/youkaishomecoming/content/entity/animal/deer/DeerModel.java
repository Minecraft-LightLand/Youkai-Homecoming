...
public class DeerModel extends AgeableHierarchicalModel<DeerEntity> {
	...
	public void setupAnim(DeerEntity e, float limbSw, float limbSA, float tick, float yrot, float xrot) {
		...
		horn.visible = e.isHorned();
		this.head.yRot = yrot * (float) (Math.PI / 180.0);
		this.head.xRot = xrot * (float) (Math.PI / 180.0);
		if (e.isPanic()) {
			this.animateWalk(DeerAnimation.RUN, limbSw, limbSA, 2f, 2.5f);
		} else {
			this.animateWalk(DeerAnimation.WALK, limbSw, limbSA, 2f, 2.5f);
		}
		this.animate(e.states.relaxStart, DeerAnimation.RELAX_START, tick);
		this.animate(e.states.relaxDur, DeerAnimation.RELAX, tick);
		this.animate(e.states.relaxEnd, DeerAnimation.RELAX_END, tick);
		this.animate(e.states.eat, DeerAnimation.EAT, tick);
		this.animate(e.states.attack, DeerAnimation.ATTACK, tick);
		this.animate(e.states.smell, DeerAnimation.SMELL, tick);
	}
